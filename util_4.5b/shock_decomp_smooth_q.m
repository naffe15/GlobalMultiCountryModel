function out = shock_decomp_smooth_q(xparam1, T0, ex_names_, leg, vname, no_initial_effect, texvname, nfrcst, init_names_, file_name)
...
if options_.TeX
    if flag_init
        fidTeX = fopen([M_.fname '_Shock_Decomposition_q.TeX'],'w+');
    else
        fidTeX = fopen([M_.fname '_Shock_Decomposition_q.TeX'],'a+');
    end
end
...
for j=1:length(vname),
    ...
    % %
    % %
    % % %     MAP(1:fix(nbcmpts/2),3)=max(-2*colorrange(1:fix(nbcmpts/2))+1,0);
    % %     MAP0(:,3)=-colorrange+1;
    % %     MAP0(:,2)=1-2*abs(colorrange-0.5);
    % %     MAP0(:,1)=colorrange;
    % % %     MAP(fix(nbcmpts/2):end,1)=max(2*colorrange(fix(nbcmpts/2):end)-1,0);
    % %
    % %     %   MAP = jet(size(sdec,1));
    % %
    % %     MAP1=ones(nbcmpts,3);
...
%        set(hleg,'position',[0.35 0.05 0.3 0.3],'units','normalized')
        ...
        if ~no_initial_effect,
            hold on, h1=plot(as(indx,kpoint),'k-d');
            set(h1,'MarkerFaceColor', 'k')
        end
        hold on, h1=plot(inno,'k:*');
        %   if nargin==7
        %       title(['quarter on quarter ',texvname{j},' innovations'],'interpreter','tex')
        %   else
        tit1 = ['quarter to quarter ',vname{j},' innovations'];
        ...
        dyn_saveas(gcf,[M_.fname,'_shock_dec_qoq_',vname{j},'_vs_',num2str(ngroups0),'Shocks',file_name],options_.nodisplay,options_.graph_format)
...
%       set(hleg,'position',[0.35 0.05 0.3 0.3],'units','normalized')
        ...
        set(gca,'yticklabel',num2str(get(gca,'ytick')'+get_mean(vname{j}),'%4.2g'))
        dyn_saveas(gcf,[M_.fname,'_shock_dec_qoq_',vname{j},'_vs_',num2str(ngroups0),'Shocks','_Init',file_name],options_.nodisplay,options_.graph_format)
    end
    ...
    for i=1:ngroups0+1+no_initial_effect,
        ...
        if length(get(gca,'Xtick'))>5
            set(gca,'Xtick',[5:16:length(kpoint)])
            set(gca,'Xticklabel',TT(kpoint(5):16:end))
        else
            set(gca,'Xticklabel',floor(TT(kpoint(get(gca,'Xtick')))));
        ...
        a0(4)=max(a(4),a0(4));
        set(gca,'ylim',a0(3:4))
        hold on, h1=plot(as(indx,kpoint),'k-');
    ...
    if ~no_initial_effect,
        dyn_saveas(gcf,[M_.fname,'_shock_dec_qoq_',vname{j},'_vs_',num2str(ngroups0),'Shocks','_Init_Detail',file_name],options_.nodisplay,options_.graph_format)
    else
        ...
        % hold on, h1=plot(as(indx,kpoint),'k-');
        % set(h1,'MarkerFaceColor', 'k','linewidth',2)
        dyn_saveas(gcf,[M_.fname,'_shock_dec_qoq_',vname{j},'_vs_',num2str(ngroups0),'Shocks','_Detail',file_name],options_.nodisplay,options_.graph_format)
    end
    tempshockdecom=['qoq_',vname{j},'_vs_',num2str(ngroups0),'_Shocks'];
    eval([tempshockdecom,'.Decomp','=sdec;']);
    eval([tempshockdecom,'.Init_Decomp','=sdec2;']);
    eval([tempshockdecom,'.Smooth','=inno;']);
    eval([tempshockdecom,'.Init_Smooth','=as(indx,kpoint);']);
    eval([tempshockdecom,'.Shocks','=ex_names_;']);
    eval([tempshockdecom,'.Names','=leg;']);
    eval([tempshockdecom,'.InitialTime','=T0;']);
    TagName = [M_.fname,'_QoQ_ShockDecomp_vs_',num2str(ngroups0),'Shocks',file_name];
    ...
    if options_.TeX
        ...
        if no_initial_effect,
            tempfilename=[M_.fname,'_shock_dec_qoq_',vname{j},'_vs_',num2str(ngroups0),'Shocks',file_name];
            ...
            fprintf(fidTeX,'\\caption{Shock Decomposition quarter on quarter $%s$ innovations}',texvname{j});
            fprintf(fidTeX,'\\label{Fig:Shock Decomposition:%s}\n',int2str(j));
            fprintf(fidTeX,'\\end{figure}\n');
            fprintf(fidTeX,' \n');
            
            
            tempfilename=[M_.fname,'_shock_dec_qoq_',vname{j},'_vs_',num2str(ngroups0),'Shocks','_Detail',file_name];
            ...
            tempfilename=[M_.fname,'_shock_dec_qoq_',vname{j},'_vs_',num2str(ngroups0),'Shocks','_Init',file_name];
            ...
            tempfilename=[M_.fname,'_shock_dec_qoq_',vname{j},'_vs_',num2str(ngroups0),'Shocks','_Init_Detail',file_name];
    ...
    if (flag_init && j==1) || isempty(ls([M_.fname,'_QoQ_ShockDecomp.mat']))
        %         delete  *shock_dec*qoq*.fig
        %         delete  *shock_dec*qoq*.eps
        save([M_.fname,'_QoQ_ShockDecomp'],tempshockdecom);
    else
        save([M_.fname,'_QoQ_ShockDecomp'],tempshockdecom,'-append');
    end
end
