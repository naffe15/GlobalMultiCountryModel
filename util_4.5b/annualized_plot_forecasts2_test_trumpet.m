function  AnnForecastPlot = annualized_plot_forecasts2(pplotvar, q2a, M_, options_, oo_, T, Tlim, nplots, pplottitles, occbin, cfrcst, frcst_ext, nfrcst_ext, ecfin_yes, assmpt_range2)
...
for j=1:length(pplotvar)
    [forecastedvar(:,j,:), smoothedvar(:,j), steadyvar(j), tmp] = make_avar(oo_.forecast,SmoothedVariables,options_.forecast,forecast_,npreamble,t0,pplotvar{j},q2a(j));    % unconditional forecasts
    ...
    if assmpt_range_yes == 1
        ...
        for jjj=1:size(M_.jrc.assmpt_range_vars,1)
            ...
            for jj=1+(jjj-1)*assmpt_range:jjj*assmpt_range
                smoothvar_range_assmpt(:,j,jj) = make_avar(oo_.jrc.frcst_exog_assmpt_range.( M_.jrc.cfrcst_range_name{jj} ).SmoothedVariables,SmoothedVariables,options_.forecast,forecast_,npreamble,t0,pplotvar{j},q2a(j));    % unconditional forecasts
            end
                    ...
                    if j2 ~= j0 
                        for j3 = 1:assmpt_range
                           for j4 = 1:assmpt_range
                               jj = jj+1;
                                 smoothvar_range_assmpt(:,j,jj)= make_avar(oo_.jrc.frcst_exog_assmpt_range.( M_.jrc.cfrcst_range_name{jj} ).SmoothedVariables,SmoothedVariables,options_.forecast,forecast_,npreamble,t0,pplotvar{j},q2a(j));
                           end
                        end
                    end
                end
    ...
    if bvar_yes == 1 && isempty(strmatch(pplotvar{j},fieldnames(oo_.bvar.forecast.no_shock.Mean)))==0
        forecastedbvar(:,j,:) = make_avar(oo_.bvar.forecast.no_shock,SmoothedVariables,options_.forecast,forecast_,npreamble,t0,pplotvar{j},q2a(j));    % unconditional forecasts
    end
    if exopath_yes == 1
        forecastedexop(:,j,:) = make_avar(oo_.jrc.forecast_exo_path,SmoothedVariables,options_.forecast,forecast_,npreamble,t0,pplotvar{j},q2a(j));    % unconditional forecasts
    end
    if endopath_yes == 1
        forecastedendop(:,j,:) = make_avar(oo_.jrc.forecast_endo_path,SmoothedVariables,options_.forecast,forecast_,npreamble,t0,pplotvar{j},q2a(j));    % unconditional forecasts
    end
    if occbin_yes == 1
        forecastedoccb(:,j,:) = make_avar(oo_.occbin_forecast,SmoothedVariables,options_.forecast,forecast_,npreamble,t0,pplotvar{j},q2a(j));    % unconditional forecasts
    end
...
%     set(h(1),'FaceColor',[1 1 1])
    ...
    if isfield(oo_,'MeanForecast')
        ...
        my_forecastedvar = make_avar(oo_.MeanForecast,SmoothedVariables,my_qforecast_,my_forecast_,npreamble0,t0,pplotvar{j},q2a(j));    % unconditional forecasts
    ...
    % ECFIN forecast
    if frcst_ext == 0
        if ecfin_yes == 1 && assmpt_range_yes ~= 2
    ...
    if assmpt_range_yes == 2
        ...
        for jj=1+(jjj-1)*length(M_.jrc.assmpt_range_vars):jjj*length(M_.jrc.assmpt_range_vars)
            ...
            if jj == 1+(jjj-1)*length(M_.jrc.assmpt_range_vars)
                ...
                %AnnForecastPlot(j).assmptrangeplot.([M_.jrc.assmpt_range_vars{jj0} '_' M_.jrc.assmpt_range_vars{jj2} '_High High']) = assmpt_range_plot;
            elseif jj == 1+(jjj-1)*length(M_.jrc.assmpt_range_vars)+1
                ...
                %AnnForecastPlot(j).assmptrangeplot.([M_.jrc.assmpt_range_vars{jj0} '_' M_.jrc.assmpt_range_vars{jj2} '_High Low']) = assmpt_range_plot;
            elseif jj == 1+(jjj-1)*length(M_.jrc.assmpt_range_vars)+2
                ...
                %AnnForecastPlot(j).assmptrangeplot.([M_.jrc.assmpt_range_vars{jj0} '_' M_.jrc.assmpt_range_vars{jj2} '_Low High']) = assmpt_range_plot;
            elseif jj == 1+(jjj-1)*length(M_.jrc.assmpt_range_vars)+3
                ...
                %AnnForecastPlot(j).assmptrangeplot.([M_.jrc.assmpt_range_vars{jj0} '_' M_.jrc.assmpt_range_vars{jj2} '_Low Low']) = assmpt_range_plot;
            end
        end
...
function [forecastedvar, smoothedvar, steadyvar, ytmp_aux_smooth] = make_avar(forecast,SmoothedVariables,qforecast_,forecast_,npreamble,t0,pplotvar,q2a,HPDplot)
