function posterior_shock_decomp_smooth_2(M_, options_, oo_, estim_params_, T0, ex_names_, leg, vname, texvname, file_name)
...
for j=1:length(temp_smooth_file_list),
    if isempty(strfind(temp_smooth_file_list(j).name,'smoothed')),
        jfile=jfile+1;
        smooth_file_list(jfile)=temp_smooth_file_list(j);
    end
end
...
for j=1:length(param_file_list),
    ...
    stock_decomp=zeros(length(vname),size(ex_names_,1)+1,length(kpoint4),size(stock_params.stock,1));
...
%         as=as(oo_.dr.inv_order_var,:);
        ...
        for jx=1:length(vname),
            ...
            sdec22(jx,:,:)=[squeeze(sdec(jx,:,:)); dump2+sum(squeeze(sdec0(jx,:,:)),1)];
        end
        stock_decomp(:,:,:,i) = sdec22;
    end
    stock0=stock;
    stock=stock_decomp;
    save([DirectoryName,filesep,M_.fname,'_decomp_yoy_vs_',num2str(size(ex_names_,1)),'Shocks',file_name,'_',int2str(j)],'stock')
    stock=stock0;
    clear stock0;
end
stock1 = zeros(length(vname),size(ex_names_,1)+1,length(kpoint4),B);
k = 0;
for file = 1:length(param_file_list)
    load([DirectoryName,filesep,M_.fname,'_decomp_yoy_vs_',num2str(size(ex_names_,1)),'Shocks',file_name,'_',int2str(file)],'stock')
...
for j=1:length(vname),
    ...
    a=axis;
    if a(3)+4*SSMean(j)<0 && a(4)+4*SSMean(j)>0,
        plot(a(1:2),SSMean(j)*4*[-1 -1],'k--','linewidth',1)
        ytick=get(gca,'ytick');
        ytick1=ytick-SSMean(j)*4;
    ...
    set(gca,'yticklabel',num2str(get(gca,'ytick')'+SSMean(j)*4,'%4.2g'))
    dyn_saveas(gcf,[OutDirectoryName,filesep,M_.fname,'_posterior_shock_dec_yoy_',vname{j},'_vs_',num2str(size(ex_names_,1)),'Shocks',file_name,'_Init'],options_.nodisplay,options_.graph_format)
    ...
    for i=1:size(ex_names_,1)+1,
        ...
        subplot(nrow,ncol,i), 
        plot(distr','color',MAP(i,:)), title(leg0{i}),
    ...
    dyn_saveas(gcf,[OutDirectoryName,filesep,M_.fname,'_posterior_shock_dec_yoy_',vname{j},'_vs_',num2str(size(ex_names_,1)),'Shocks',file_name,'_Init_Detail'],options_.nodisplay,options_.graph_format)
    if options_.TeX
        
        tempfilename=[OutDirectoryName,filesep,M_.fname,'_posterior_shock_dec_yoy_',vname{j},'_vs_',num2str(size(ex_names_,1)),'Shocks',file_name,'_Init'];
