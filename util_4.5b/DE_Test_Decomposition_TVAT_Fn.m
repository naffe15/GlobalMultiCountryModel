function DE_Test_Decomposition_TVAT_Fn(M_)
global options_
...
l3='LTR and BEN';
C4 =  +etvat(2:end).*exp(elpc(2:end))./exp(elpy(2:end)).*exp(elc(2:end))+etvat(2:end).*exp(elphouse(2:end))./exp(elpy(2:end)).*exp(elihouse(2:end));
...
ss4=get_mean('DE_TVAT')*get_mean('DE_C')+get_mean('DE_TVAT')*get_mean('DE_IHOUSE');
