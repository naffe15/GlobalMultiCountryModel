...
elseif ~isempty(strmatch('mode', comp_type, 'exact')) && length(folders)==2
   for i=1:length(folders)
       cd(folders{i})
       tmp=load('gemc_auto_mode','xparam1','parameter_names');
...
if strmatch('irfs', comp_type, 'exact')
    ...
    if tmp(end)
        ...
        if tmp2(end)
            isexpansionary=eval(['oo',int2str(1),'_.irfs.',char(strcat('LYOBS_',var_list{1}(end-1:end))),'_',shockid,';']);
        else
            isexpansionary=eval(['oo',int2str(1),'_.irfs.',char(strcat('LYOBS_',varargin{2}(1))),'_',shockid,';']);
        end
    else
        tmp2=isletter(var_list{1});
        if tmp2(end)
            isexpansionary=eval(['oo',int2str(1),'_.irfs.',char(strcat('LYOBS_',var_list{1}(end-1:end))),'_',char(strcat(shockid,varargin{2}(1))),';']);
        else
            isexpansionary=eval(['oo',int2str(1),'_.irfs.',char(strcat('LYOBS_',varargin{2}(1))),'_',char(strcat(shockid,varargin{2}(1))),';']);
        end
        ...
        subplot(3,3,j)
            ...
            for i=1:length(folders)
                ...
                if tmp(end)
                    ...
                    if tmp2(end)
                        IRF=eval(['oo',int2str(i),'_.irfs.',char(var_list{j+(ifig-1)*9}),'_',shockid,';']);
                    else
                        IRF=eval(['oo',int2str(i),'_.irfs.',char(strcat(var_list{j+(ifig-1)*9},varargin{2}(i))),'_',shockid,';']);
                    end
                else
                    tmp2=isletter(var_list{j+(ifig-1)*9});
                    if tmp2(end)
                        IRF=eval(['oo',int2str(i),'_.irfs.',char(var_list{j+(ifig-1)*9}),'_',char(strcat(shockid,varargin{2}(i))),';']);
                    else
                        IRF=eval(['oo',int2str(i),'_.irfs.',char(strcat(var_list{j+(ifig-1)*9},varargin{2}(i))),'_',char(strcat(shockid,varargin{2}(i))),';']);
                    end
...
%     Mtemp_=M1_;
    for i=1:nplots
       ...
       eval(['options',int2str(i),'_.plot_shock_decomp.detail_plot = 0;']);
        ...
        eval(['options',int2str(i),'_.plot_shock_decomp.type = ''yoy'';']);
        eval(['options',int2str(i),'_.plot_shock_decomp.nodisplay = 0;']);
        eval(['options',int2str(i),'_.plot_shock_decomp.write_xls = 0;']);
        eval(['options',int2str(i),'_.plot_shock_decomp.plot_init_date = [];']);
    ...
    for i=1:nplots^2
        ...
        eval(['options',int2str(i-nplots*(j-1)),'_.plot_shock_decomp.type = ''yoy'';']);
...
%             [i_var,nvar] = varlist_indices(var_list,M1_.endo_names);
            ...
            if i<=(nplots*j)
                ...
                compare_graph_decomp(z,shock_names,M1_.endo_names,1,my_initial_date,M1_,options1_);
