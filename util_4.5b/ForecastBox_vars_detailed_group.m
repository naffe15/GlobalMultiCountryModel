...
box2(3:size(Tab_Data2,1), 1) = F_Headers(1,2:size(Tab_Data2,1)-1);
...
if ~ismac
    ...
    xlswrite_MACOS(sprintf('%s/ForecastBox.xls', path), box, sheet)
    xlswrite_MACOS(sprintf('%s/ForecastBox.xls', path), box2(1:end,1:4), sheet,'A24')
    xlswrite_MACOS(sprintf('%s/ForecastBox.xls', path),    F_Headers(6:end,:), sheet,'A50')
end
...
if ~isempty(exassmptyes)
    ...
    if ~ismac
        ...
        xlswrite_MACOS(sprintf('%s/ForecastBox.xls', path), box_assmpt, sheet);
        xlswrite_MACOS(sprintf('%s/ForecastBox.xls', path), box_assmpt2(1:end,1:6), sheet,'A24')
        xlswrite_MACOS(sprintf('%s/ForecastBox.xls', path),    F_Headers(6:end,:), sheet,'A50')
    end
