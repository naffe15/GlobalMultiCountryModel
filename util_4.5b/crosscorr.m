function varargout = crosscorr(Series1 , Series2 , nLags , nSTDs, prefilter)
...
%
%   prefilter - 1 if mean has to betaken out; 0 - assume the data are
%   without the mean
%
% Outputs:
...
if (nargin >= 5) & ~isempty(prefilter),
    if (prefilter ~=0) && (prefilter ~=1),
      error('prefilter' , ' prefilter must be 0 or 1');
    end
else
    prefilter=1;
end
