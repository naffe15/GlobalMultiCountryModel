...
function GM_plot_IRF_2(xname, vargin, s1, texname, nr, nc, dirname, labels, Tend, init)
...
for i=1:length(vargin),
    ...
    if isempty(itemp)
        ...
        if ~isempty(k)
            ...
            countries=[countries;vargin{i}(k(1)+1:k(end)-1)];    
            end
            ic=ic+1;
        end
    end
...
for i=1:length(vargin)
    ...
    if isempty(k) % Global variable
        ...
        coindxs(i,j) = 1;
            ...
            varswithoutco{i}=vargin{i}(1:k(1)-1);
            if varswithoutco{i}=='LPOILOBS'
                varswithoutco{i}=vargin{i}([1:k(1)-1 k(end):end]);
            end
            secondco{i}=vargin{i}(k(end)+1:end);
            firstco{i}=vargin{i}(k(1)+1:k(end)-1);
        j=1;
        tempi=strfind(vargin{i}(k(1)+1:k(end)-1),countries{j});
        while isempty(tempi)
            j=j+1;
            tempi=strfind(vargin{i}(k(1)+1:k(end)-1),countries{j});
        end
    ...
    for j=1:length(uvars) % For all variables
        ...
        if sum(uvarsco(j,:))>0
            ...
            for c=1:size(uvarsco,2) % For all countries
                if uvarsco(j,c)==1,
                    if  isempty(secondco{j}) 
                         name = [uvars{j} '_' countries{c}  '_' deblank(M_.exo_names(iexo(i),:))];
                    else
                        if  ~isempty(strfind(uvars{j},'_'))
                            uvars_temp{j}=uvars{j}(1:strfind(uvars{j},'_')-1);
                         %name = [uvars{j} '_' countries{c} '_' secondco{j} '_' deblank(M_.exo_names(iexo(i),:))];
                         name = [uvars_temp{j} '_' countries{c} '_' secondco{j} '_' deblank(M_.exo_names(iexo(i),:))];
                        end
                    end
                    ...
                    if texnameflag,
                        if  isempty(secondco{j}) 
                            inx = strmatch([uvars{j} '_' countries{c} ],vargin);                       
                             ...
                             if  ~isempty(strfind(uvars{j},'_'))
                                uvars_temp{j}=uvars{j}(1:strfind(uvars{j},'_')-1);
                            %inx = strmatch([uvars{j} '_' countries{c} '_' secondco{j}],vargin);    
                            inx = strmatch([uvars_temp{j} '_' countries{c} '_' secondco{j}],vargin);    
                             end
                        end
        ...
        if oktoplot==1
            ...
            height = (1-bottom_space-top_space-hoffset*(nr-1))/nr;
            width = (1-2*left_space-woffset*(nc-1))/nc;

            for isp = 1:nr*nc
                row=ceil(isp/nc);
                col= isp-(row-1)*nc;
                left(isp)= left_space + woffset*(col-1) +width*(col-1);
                bottom(isp)=bottom_space + hoffset*(nr-row) +height*(nr-row);
    
            end

           
                subplot('Position',[left(nplo) bottom(nplo) width height]) 
            ...
            if globalvariable==1
                ...
                if max(abs(MeanIRF)) > 1e-6
                    plot([1:Tend]/xscale,MeanIRF(1:Tend)*vscale(j)*exoscale(i),'color',colors(1,:),'linewidth',1.5,'Linestyle',':');
                end
            else
                for c=1:size(uvarsco,2) % For all countries'
                    if uvarsco(j,c)==1
            ...
            if options_.TeX==0,                
                ...
                title(fname,'interpreter','none', 'FontSize', 14)
            else
