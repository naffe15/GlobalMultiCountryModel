...
%         end
        ...
        eval(['rmse(j) = sqrt(mean((oo_.UpdatedVariables.',vj,'(istart:end)-oo_.FilteredVariables.',vj,'(istart-1:options_.nobs-1)).^2));'])
        eval(['vv(:,j) = (oo_.UpdatedVariables.',vj,'(istart:end)-oo_.FilteredVariables.',vj,'(istart-1:options_.nobs-1));'])
        eval(['r2(:,j) = 1-sum(vv(:,j).^2)/sum((oo_.UpdatedVariables.',vj,'(istart:end)-trend(j,istart:end)'').^2);'])
