function [bigVAR, yy1, yy2, xm]=vecm1(xx, xvec, beta, nex, nk, nlag)
global opts_vecm_
...
for j=1:nlag,
    ...
    ind_ex_ = [ind_ex_ nex+nvar*(j-1)];
end
yyy = xx(nlag+1:end,nex) ;
bigVAR=zeros(nvar,nvar*nlag+nvec);
for j=1:length(nex),
...
if ~isempty(j0),   
    if length(i0)==1,
        ...
        bigVAR(nex(i0),[ind_ex_ nvar*nlag+j0])= ((xxxx'*xxxx)\sum(xxxx.*kron(yyy(:,i0),ones(1,size(xxxx,2))))')';        
    elseif ~find(diff(i0))
        xxxx = [xxx xvec(1:end-nlag,j0)];
        bigVAR(nex(i0),[ind_ex_ nvar*nlag+j0])= ((xxxx'*xxxx)\sum(xxxx.*kron(yyy(:,i0),ones(1,size(xxxx,2))))')';        
    else
        ...
        for j=1:length(ii),
            ...
            bigVAR(nex(ii(j)),[ind_ex_ nvar*nlag+j0(jj)])= ((xxxx'*xxxx)\sum(xxxx.*kron(yyy(:,ii(j)),ones(1,size(xxxx,2))))')';        
        end
    end
end
end
...
ind_vec_ = [nvar*nlag+1:nvar*nlag+nvec];
...
dumVECM=[];
for j=1:nlag,
    dumVECM = [dumVECM beta*bigVAR(:,1+nvar*(j-1):nvar*j)];
end
dumVECM=[dumVECM  eye(size(beta,1))+beta*bigVAR(:,ind_vec_) ];

if nlag>1,
bigVAR = [bigVAR; ...
        [eye(nvar*(nlag-1)) zeros(nvar*(nlag-1), nvec+nvar)]; ...   % more than 1 lag rows
...
xxx=[xxx xvec];
yy1=zeros(size(xxx,1)+nk,nvar+nvec,nk);
% for j=1:length(x00),
%     yy1(:,j,:)=yy1(:,j,:).*x00(j);
% end
x00=[x0 x0vec];
...
for j=1:nk,
   ...
   yy1(j+1:size(xxx,1)+j,:, j)=dum(:,[[1:nvar] ind_vec_])+ ones(size(xxx,1),1)*[x0 x0vec];
end
...
%        dum=[1 dum];
   ...
   yy2(j,:)=dum([[1:nvar] ind_vec_])+[x0 x0vec];
end
...
if max(sort(abs(eig(bigVAR(:,:)))))>1,
    ...
    disp(['WARNING: the VECM with ',txtmean,' mean is unstable!!!'])
end