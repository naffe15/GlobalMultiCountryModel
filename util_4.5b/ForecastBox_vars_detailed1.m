...
othersCO  = [ F_Data(rowNo,indx) CF_Data(rowNo,indx) ];
indx = strmatch('Others + Initial Values',F_Headers(1,:));
others_initval  = [ F_Data(rowNo,indx) CF_Data(rowNo,indx) ];

indx = [strmatch('Inventories',F_Headers(1,:)) ];
if ~isempty(indx)
    Inventories         = [ F_Data(rowNo,indx) CF_Data(rowNo,indx) ];
else
    Inventories = [nan nan];
end
indx = [strmatch('TB services',F_Headers(1,:)) ];
if ~isempty(indx)
    TB_services         = [ F_Data(rowNo,indx) CF_Data(rowNo,indx) ];
else
    TB_services = [nan nan];
end
indx = [strmatch('Shocks REA',F_Headers(1,:)) ];
if ~isempty(indx)
    Shocks_REA         = [ F_Data(rowNo,indx) CF_Data(rowNo,indx) ];
else
    Shocks_REA = [nan nan];
end
...
if exist('NonOil')
    ...
    Tab_Data_tmp2 = [ TFP; Fiscal; Monetary; PriceMkup; FX; Private_savings; Investment; Wage_Markup; Labor_cost; othersCO; US; RoW;Shocks_REA;Commodities_demand; Trade; Oil; NonOil; Safety; others_initval ; [NaN NaN]];
else
    ...
    Tab_Data_tmp2 = [ TFP; Fiscal; Monetary; PriceMkup; FX; Private_savings; Investment; Wage_Markup; Labor_cost; othersCO; US; RoW; Shocks_REA;Commodities_demand; Trade;Oil; Safety; others_initval ; [NaN NaN]];
...
if ~isempty(exassmptyes)
    ...
    indx = strmatch('Other shocks ',F_Headers(1,:));
    othersCOassmpt   = [ Fassmpt_Data(rowNo,indx)  ];
    indx = strmatch('Others + Initial Values',F_Headers(1,:));
    othersintassmpt  = [ Fassmpt_Data(rowNo,indx) ];
    
    indx = [strmatch('Inventories',F_Headers(1,:)) ];
    if ~isempty(indx)
        Inventoriesassmpt         = [ F_Data(rowNo,indx) ];
    else
        Inventoriesassmpt = [nan ];
    end
    indx = [strmatch('TB services',F_Headers(1,:)) ];
    if ~isempty(indx)
        TB_servicesassmpt         = [ F_Data(rowNo,indx)  ];
    else
        TB_servicesassmpt = [nan ];
    end
    indx = [strmatch('Shocks REA',F_Headers(1,:)) ];
    if ~isempty(indx)
        Shocks_REAassmpt         = [ F_Data(rowNo,indx) ];
    else
        Shocks_REAassmpt = [nan];
    end
    ...
    if exist('NonOil')
        ...
        Tab_Data_tmp_assmpt2 = [ TFPassmpt; Fiscalassmpt; Monetaryassmpt; PriceMkupassmpt; FXassmpt; Private_savingsassmpt; Investmentassmpt; Wage_Markupassmpt; Labor_costassmpt; othersCOassmpt; USassmpt; RoWassmpt;Shocks_REAassmpt;Commodities_demandassmpt; Tradeassmpt; Oilassmpt; NonOilassmpt; Safetyassmpt; othersintassmpt ; SmoothVarAssmpt];
    else
        ...
        Tab_Data_tmp_assmpt2 = [ TFPassmpt; Fiscalassmpt; Monetaryassmpt; PriceMkupassmpt; FXassmpt; Private_savingsassmpt; Investmentassmpt; Wage_Markupassmpt; Labor_costassmpt; othersCOassmpt; USassmpt; RoWassmpt; Shocks_REAassmpt;Commodities_demandassmpt; Tradeassmpt; Oilassmpt; Safetyassmpt; othersintassmpt ; SmoothVarAssmpt];
...
if exist('NonOil')
    box2(3:size(Tab_Data2,1)+2, 1)= {[NaN],[NaN], 'TFP', 'Fiscal', 'Monetary', 'Price Mrk-up', 'Bond premium ', 'Private savings', 'Investment risk', 'Wage Mrk-up', 'Labor cost','other shocks country', 'US','shocks RoW','shocks REA','Commodities demand', 'Trade', 'Oil', 'NonOil', 'Flight to safety', 'Others+Init. val.', 'Smooth var'};
else
    box2(3:size(Tab_Data2,1)+2, 1) = { [NaN],[NaN],'TFP', 'Fiscal', 'Monetary', 'Price Mrk-up', 'Bond premium', 'Private savings', 'Investment risk', 'Wage Mrk-up', 'Labor cost','other shocks country','US','shocks RoW','shocks REA','Commodities demand', 'Trade', 'Oil','Flight to safety', 'Others+Init. val.', 'Smooth var'};
end
...
if ~isempty(exassmptyes)
    ...
    if exist('NonOil')
        box_assmpt2(3:size(Tab_Data_assmpt2,1)+2, 1) = {[NaN],[NaN], 'TFP', 'Fiscal', 'Monetary', 'Price Mrk-up', 'Bond premium ', 'Private savings', 'Investment risk', 'Wage Mrk-up', 'Labor cost','other shocks country', 'US','shocks RoW','shocks REA','Commodities demand',  'Trade', 'Oil', 'NonOil', 'Flight to safety', 'Others+Init. val.', 'Smooth var'};

    else
        box_assmpt2(3:size(Tab_Data_assmpt2,1)+2, 1) ={ [NaN],[NaN],'TFP', 'Fiscal', 'Monetary', 'Price Mrk-up', 'Bond premium', 'Private savings', 'Investment risk', 'Wage Mrk-up', 'Labor cost','other shocks country','US','shocks RoW','shocks REA','Commodities demand',  'Trade', 'Oil','Flight to safety', 'Others+Init. val.', 'Smooth var'};
    end
