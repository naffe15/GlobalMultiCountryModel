...
function compare_scenarios(path, oo_, oo1, country, assmpt)
...
%     indx = find(contains(cellstr(char(oo_.jrc.forecastA.VarName)),char(varsT1(i).name))==1);
    ...
    if isempty(indx)
        ...
        table.(char(varsT1(i).name)).value= oo1.jrc.forecastA(indx(1)).exogassmpt(end-2:end) ;
        table.(char(varsT1(i).name)).name= char(varsT1(i).name) ;
        table.(char(varsT1(i).name)).def= char(varsT1(i).def) ;
...
box_values3 = [[NaN, NaN, NaN] ;table.(['PHICVATA_' country]).value'; table.(['PHIGA_' country]).value'; table.(['PHIIGA_' country]).value'; table.(['PHIIA_' country]).value'; table.(['PHIXA_' country]).value'; table.(['PHIMTOTA_' country]).value'; table.(['PHIYOBSA_' country]).value'];
...
box_values9 = [[NaN, NaN, NaN] ;table.(['RGYA_' country]).value';  table.(['BGYA_' country]).value';  table.(['DFETOTA_' country]).value';  table.(['GTA_' country]).value'];
...
if length(assmpt)>31
xlswrite(sprintf('%s\\Compare_Scenarios.xls', path), box, assmpt(1:31))
else
xlswrite(sprintf('%s\\Compare_Scenarios.xls', path), box, assmpt)
end
