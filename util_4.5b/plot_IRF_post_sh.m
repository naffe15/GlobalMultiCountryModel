function plot_IRF_post_sh(xname, vargin, s1, texname, nr, nc, dirname)
...
for i=1:length(iexo),
    ...
    for j=1:length(vargin),
        ...
        if max(abs(MeanIRF)) > 1e-6 ,
            ...
            subplot(nr,nc,nplo)
            patch([[1:irf_length] [irf_length:-1:1]],[HPDsup; HPDinf(end:-1:1)]'*100,[0.75 0.75 0.75])
            hold on
            plot([1 irf_length],[0 0],'-r','linewidth',0.5);
            %       for k = 1:9
            %         plot(1:irf_length,DistribIRF(:,k),'-g','linewidth',0.5)
            %       end
            plot(1:irf_length,MeanIRF(:)*100,'-k','linewidth',1)
            xlim([1 irf_length]);
            hold off
            if options_.TeX==0,
