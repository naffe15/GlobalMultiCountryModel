...
function GM_plot_IRF(xname, vargin, s1, texname, nr, nc, dirname, labels, Tend, init)
            ...
            if globalvariable==1
                ...
                for c=1:size(uvarsco,2) % For all countries
                    if uvarsco(j,c)==1
                        ...
                        if max(abs(MeanIRF)) > 1e-6
                            plot([1:Tend]/xscale,MeanIRF(1:Tend)*vscale(j)*exoscale(i),'color',colors(c+1,:),'linewidth',1);
                        end
                    end
                end
            end
