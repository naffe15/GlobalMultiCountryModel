...
xxx = dataset(options_.first_obs:options_.first_obs+options_.presample-1,:);
xxx = [ones(size(xxx,1),1) xxx];
yyy = dataset(options_.first_obs+1:options_.first_obs+options_.presample,:);
for j=1:size(yyy,2),
  xx = xxx(:,[1 j+1]);
  bigVAR = (inv(xx'*xx)*sum(xx.*kron(yyy(:,j),ones(1,size(xx,2))))');
  sig(j)=std(yyy(:,j)-xx*bigVAR);
end
