...
%    end
    ...
    % 6=Private savings shock EA 7=Investment risk premium EA	8=Wage Mark-up EA	
    % 9= Labor cost shock EA	10=Other shocks EA	11=Shocks RoW	12=Trade shocks	
...
% Additional shocks for detailed Table ForecastBox (no assumptions)
  ...
  indx = strmatch('Others + Initial Values',F_Headers(1,:));
   others_initval  = [ F_Data(rowNo,indx) CF_Data(rowNo,indx) ];
    ...
    %Table ForecastBox with assumptions
    if ~isempty(exassmptyes)
...
%         end
    ...
    box_assmpt2 = cell(19, 6);
        ...
        if exist('NonOil')
            box_assmpt2(3:size(Tab_Data_assmpt2,1)+2, 1) = { 'TFP', 'Fiscal', 'Monetary', 'Price Mrk-up', 'Bond premium ', 'Private savings', 'Investment risk', 'Wage Mrk-up', 'Labor cost','other shocks REA', 'US','shocks RoW', 'Trade', 'Oil', 'NonOil', 'Flight to safety', 'Others+Init. val.', 'Smooth var'};
        else
    ...
    xlswrite(sprintf('%s\\ForecastBox.xls', path), box, sheet,'A1')
    xlswrite(sprintf('%s\\ForecastBox.xls', path), box2(1:end,1:4), sheet,'A24')

if ~isempty(exassmptyes)
    sheet = 'Assmption';
     xlswrite(sprintf('%s\\ForecastBox.xls', path), box_assmpt, sheet,'A1');
