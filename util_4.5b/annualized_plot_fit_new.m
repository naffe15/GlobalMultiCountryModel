function  [r2a, vname] = annualized_plot_fit_new(pplotvar, q2a, M_, options_, oo_, T, Tlim, nplots, pplottitles, pposterior)
...
for j=1:length(pplotvar)
    ...
    [ya, yass, gya, gyass] = ...
        quarterly2annual(oo_.UpdatedVariables.(pplotvar{j})(t0:end-forecast)-get_mean(pplotvar{j}),get_mean(pplotvar{j}),q2a(j).GYTREND0,q2a(j).type,q2a(j).islog,q2a(j).aux);
    if q2a(j).plot ==1,
        ztmp=gya+gyass;
    elseif q2a(j).plot == 2
        ztmp=ya+yass;
...
for j=1:length(pplotvar)
    ...
    plot(TQ,oo_.UpdatedVariables.(pplotvar{j})(1:length(TQ)),'linewidth',2)
    hold on,
%     for jt=1:length(oo_.UpdatedVariables.(pplotvar{j}))
    for jt=1:4:length(oo_.UpdatedVariables.(pplotvar{j})(1:length(TQ)))
